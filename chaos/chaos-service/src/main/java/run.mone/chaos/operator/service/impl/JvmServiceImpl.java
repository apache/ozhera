...
@Slf4j
@Service
public class JvmServiceImpl extends TaskBaseService {
    ...
    public String save(CreateChaosTaskBo createChaosTaskBo, PipelineBO pipelineBO, Integer taskStatus) {
        log.info("save jvm task param createChaosTaskBo={},pipelineBO={},taskStatus={}", createChaosTaskBo, pipelineBO, taskStatus);
        JvmPO jvmPO = new JvmPO();
        BeanUtils.copyProperties(pipelineBO, jvmPO);
        ChaosTask chaosTask = ChaosTask.of(createChaosTaskBo, taskStatus);
        jvmPO.setType(((JvmBO) pipelineBO).getType());
        String rule = null;
        if (jvmPO.getType().equals(JvmActionEnum.exception.type())) {
            String doing = "Integer.valueOf(\"a\");";
            rule = String.format(CmdConstant.JVM_TEMPLATE, UUID.randomUUID().toString(), jvmPO.getClName(), jvmPO.getMethodName(), doing);
        } else if (jvmPO.getType().equals(JvmActionEnum.re.type())) {
            String doing = String.format(CmdConstant.JVM_TEMPLATE_RETURN, jvmPO.getResponse());
            rule = String.format(CmdConstant.JVM_TEMPLATE, UUID.randomUUID().toString(), jvmPO.getClName(), jvmPO.getMethodName(), doing);
        }
        ...
        jvmPO.setInstanceUidAndIPList(instanceUidAndIPList);
    ...
    public String execute(ChaosTask chaosTask) {
        ...
        if (modeEnum.equals(ModeEnum.APPOINT)) {
            podList = CommonUtil.getExecutedPod(podList, chaosTask.getJvmPO().getInstanceUidAndIPList().stream().map(InstanceUidAndIP::getIp).toList());
            List<String> ips = CommonUtil.getExcludedIPs(podList, chaosTask.getJvmPO().getInstanceUidAndIPList().stream().map(InstanceUidAndIP::getIp).toList());
            if (!ips.isEmpty()) {
        ...
        grpcPodAndChannels.forEach(grpcPodAndChannel -> {
            Future<String> objectFuture = Executors.newVirtualThreadPerTaskExecutor().submit(() -> {
                ...
                try {
                    containId = CommonUtil.getContainerIdByName(grpcPodAndChannel.getPod(), chaosTask.getContainerName(), chaosTask.getProjectId(), chaosTask.getPipelineId());
                    Chaosdaemon.InstallJVMRulesRequest request = Chaosdaemon.InstallJVMRulesRequest.newBuilder().setEnterNS(true).setContainerId(containId).setPort(9288).setRule(chaosTask.getJvmPO().getRule()).build();
                    ChaosDaemonGrpc.newBlockingStub(grpcPodAndChannel.getChannel()).installJVMRules(request);
                } catch (Exception e) {
                    log.error("jvm task execute error, taskId:{}", chaosTask.getId().toString(), e);
                }
                return containId;
            });
        ...
        Executors.newVirtualThreadPerTaskExecutor().execute(() -> {
            ...
            map.put("jvmPO.instanceUidAndIPList", instanceUidAndIPList);
    ...
    public String recover(ChaosTask chaosTask) {
        ...
        List<InstanceUidAndIP> instanceUidAndIPList = chaosTask.getJvmPO().getInstanceUidAndIPList();
        ...
        grpcPodAndChannels.forEach(grpcPodAndChannel -> {
            Future<Integer> objectFuture = Executors.newVirtualThreadPerTaskExecutor().submit(() -> {
                ...
                try {
                    ...
                    ChaosDaemonGrpc.newBlockingStub(grpcPodAndChannel.getChannel()).uninstallJVMRules(request);
                } catch (Exception e) {
                    log.error("cancel jvm error, taskId:{}, ip:{}", chaosTask.getId().toString(), instance.get().getIp(), e);
                    //todo 通知该pod重启恢复
                    return 0;
                }
                return 1;
        ...
        for (int i = 0; i < futureList.size(); i++) {
            try {
                ...
                log.error("cancel jvm error, taskId:{}", chaosTask.getId().toString(), e);
                //todo 通知该pod重启恢复
            }
        }
        Map<String, Object> map = new HashMap<>();
