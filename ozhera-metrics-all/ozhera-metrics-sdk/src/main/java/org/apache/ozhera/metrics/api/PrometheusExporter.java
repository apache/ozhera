...
class PrometheusExporter {
    ...
    static void startHttpServer(int port, CollectorRegistry registry) {
        new Thread(() -> {
            LOG.info("start prometheus exporter");
            try {
                LOG.info("prometheus port: {}", port);
                InetSocketAddress addr = new InetSocketAddress(port);
                new HTTPServer(HttpServer.create(addr, 3), registry, true);
            } catch (IOException e) {
                LOG.error("start prometheus exporter error", e);
            }
        }).start();
    }
}
